							...
							else
							{
								if(data_ar && !extractLastOneComma(request.term)){
									...
									if(imgdata)
									{
										...
										var html = getImg(imgdata,keyTerm);
										if(html)
											jQuery('.ui-autocomplete').append(html);
									}
								}
					...
					function typehead_desig_comp(counter)
					{
						jQuery(function(){
						var data_ar =[];
						var tmp_data_Arr =[];
						...
						jQuery(document).on('mouseenter', '#designation_'+cntvalue, function(){   
							var skill_this = jQuery(this);         
				                if(!skill_this.hasClass('binded')){
						skill_this.addClass('binded');
					        skill_this.autocomplete({	
						source: function (request, response) {
								jQuery.trim(request.term);
								var keyTerm = extractLast(request.term);
								if(keyTerm.length < 2) return 0;
								jQuery.ajax({
									type: "GET",
									url: "http://my."+Mons.BASE_SERVER+"/tokenize.html?search="+keyTerm+"&section=designation",    
									dataType: "text",
									success: function(data){
										if(data)
										{
						...
						select: function( event, ui ) {
							...
							terms.push( "" );
							this.value = terms.join( "," );
							var idterms = splitN(jQuery( "#designationid").val());
							    idterms.pop();
    								idterms.push(ui.item.value+':'+ui.item.id);
    								idterms.push('');
    							jQuery( "#designationid" ).val(idterms.join(','))
							//Suggation(jQuery(this));
							return false;
						},
						position: {
					...
					jQuery(function(){
						...
						jQuery(document).on('mouseenter', '#company_name_'+cntvalue, function(){   
							var skill_this = jQuery(this);         
				                if(!skill_this.hasClass('binded')){
						skill_this.addClass('binded');
					        skill_this.autocomplete({	
						source: function (request, response) {
								...
								jQuery.ajax({
									type: "GET",
									url: "http://my."+Mons.BASE_SERVER+"/tokenize.html?search="+keyTerm+"&section=company",    
									dataType: "text",
									success: function(data){
										if(data)
										{
						...
						select: function( event, ui ) {
							...
							terms.push( "" );
							this.value = terms.join( "," );
							var idterms = splitN(jQuery( "#compnameid" ).val());
							    idterms.pop();
    								idterms.push(ui.item.value+':'+ui.item.id);
    								idterms.push('');
    							jQuery( "#compnameid" ).val(idterms.join(','))
							//Suggation(jQuery(this));
							return false;
						},
						position: {
...
function getImg(cachedata,keyTerm)
{
        ...
        if(imgd){
                jQuery.each(imgd, function( index, img ){
                        ...
                        if(imgarr && imgarr.length > 0){
                                ...
                                var imgvrArr = imgvr.split(',');
                                if(imgvrArr && imgvrArr.length > 0) {
                                        jQuery.each(imgvrArr, function(index, item) {
                                                 if(item.match("^"+keyTerm)){
                                                        imgFlag =1;
                                             }
                                        });
                                }
	...
	if(imgsrc){
		...
		imghtml = '<li style="border-bottom:solid 1px #bfbfbf; width:'+widthImg+'; height:100; color:#376db0;" class="bg_white"><a href="'+imghref+'" alt="'+imgalt+'" style="height:100px;"><img width="'+widthImg+'" height="100" src="'+mediapath+'/'+tempUrl+'/logos/'+imgsrc+'" /></a></li>';
	}
	return imghtml;
