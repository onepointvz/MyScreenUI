...
function loadData()
{
	...
	if((pagename == 'create' && cid != 1) || (pagename == 'stage2' && cid == 1) ||(pagename == 'fornation'))
	{
		initNationality();	
	}
	initLocation();
	...
	if(COMBO_DATA.CURLOCATION_VAL || ( pagename == 'stage2' && jQuery('#dbcurLoc').val()))
	{

		var curlocaVal = COMBO_DATA.CURLOCATION_VAL;
		if(curlocaVal == '') {curlocaVal = jQuery('#dbcurLoc').val(); }
		try
                {
                      updateCombo( 'location', curlocaVal);
                }       
                catch(e)
                {       
                        // if(console && console.log) console.log("Error initializing location combo");
...
//        e.setAttribute('data-placeholder', 'Nationality');

        var o = document.createElement('option');
        o.setAttribute('value', '');
	o.innerHTML = "Nationality";
        e.appendChild(o);
...
var o1 = document.createElement('option');
                ...
                o1.innerHTML = "Saudi Arabian";
e.appendChild(o1);
var o1 = document.createElement('option');
                o1.setAttribute('value', 'AE');
                o1.innerHTML = "Emirati/UAE";
e.appendChild(o1);
//$nationlaityheader .=qq{<option value='BH' >Bahraini</option><option value='IQ' >Iraqi</option><option value='KW' >Kuwaiti</option><option value='OM' >Omani</option><option value='QA' >Qatari</option><option value ='SA'>Saudi Arabia</option><option value ='AE'>United Arab Emirates</option>};
	...
	if(glassdoor == 1)
	{
        	$('#nationality').select2({placeholder: MSG['NATIONALITY'], allowClear: true, width: "270px"});
	}
	else
	{
		$('#nationality').select2({placeholder: MSG['NATIONALITY'], allowClear: true, width: "550px"});
	}
	$('#s2id_nationality').addClass("nationality");
	$('#s2id_nationality').addClass("req");
...
function initLocation()
{
	...
	if(glassdoor == 1)
	{
		$('#location').select2({placeholder: MSG['CUR_LOC'], allowClear: true, width: "270px"});
	}
	else
	{
		$('#location').select2({placeholder: MSG['CUR_LOC'], allowClear: true, width: "550px"});
	}
	$('#s2id_location').addClass("location");
	$('#s2id_location').addClass("req");
